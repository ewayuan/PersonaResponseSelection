...
class cnnBlock(nn.Module):
    def __init__(self):
        ...
        self.affine_out = nn.Linear(256, 1)
    ...
    def forward(self, context_response_emb_similarity_matrix, persona_response_emb_similarity_matrix, \
                              context_response_attn_similarity_matrix, context_response_similarity_matrix,\
        ...
        context_response_M = torch.stack([context_response_emb_similarity_matrix, context_response_attn_similarity_matrix, context_response_similarity_matrix, context_response_word_level_attn_simialrity_matrix], dim=1)
        persona_response_M = torch.stack([persona_response_emb_similarity_matrix, persona_response_attn_similarity_matrix, persona_response_similarity_matrix, persona_response_word_level_attn_simialrity_matrix], dim=1) 
...
class ourModel (nn.Module):
    ...
    def forward(self, batch_context_emb, batch_response_emb, batch_persona_emb, \
        ...
        # Normalize
        batch_context_emb = torch.nn.functional.normalize(batch_context_emb, dim=-1) 
        batch_response_emb = torch.nn.functional.normalize(batch_response_emb, dim=-1) 
        batch_persona_emb = torch.nn.functional.normalize(batch_persona_emb, dim=-1) 

        batch_Uce_context = torch.nn.functional.normalize(batch_Uce_context, dim=-1) 
        batch_Uce_response = torch.nn.functional.normalize(batch_Uce_response, dim=-1) 
        batch_Uce_persona = torch.nn.functional.normalize(batch_Uce_persona, dim=-1) 
        ...
        context_response_emb_similarity_matrix = torch.bmm(batch_context_emb, batch_response_emb.transpose(1,2))

        persona_response_emb_similarity_matrix = torch.bmm(batch_persona_emb, batch_response_emb.transpose(1,2))
        ...
        context_attn_output = self.context_transformer(batch_context_emb, batch_context_emb, batch_context_emb, mask=context_self_mask.bool())
        response_attn_output = self.response_transformer(batch_response_emb, batch_response_emb, batch_response_emb, mask=response_self_mask.bool())
        persona_attn_output = self.persona_transformer(batch_persona_emb, batch_persona_emb, batch_persona_emb, mask=persona_self_mask.bool())


        context_response_similarity_matrix = torch.bmm(context_attn_output, response_attn_output.transpose(1,2))

        persona_response_similarity_matrix = torch.bmm(persona_attn_output, response_attn_output.transpose(1,2))
