...
class cnnBlock(nn.Module):
    ...
    def forward(self, context_response_attn_similarity_matrix, context_response_similarity_matrix,\
        ...
        context_respons_interaction_V = self.cnn_contxt_response(context_response_word_level_attn_simialrity_matrix)
        persona_response_interaction_V = self.cnn_persona_response(persona_response_word_level_attn_simialrity_matrix)

        all_concat = torch.cat([context_response_attn_V, context_response_V, \
                                persona_response_attn_V, persona_response_V, \
                                context_respons_interaction_V, persona_response_interaction_V], dim=-1)
...
class ourModel (nn.Module):
    ...
    def train_epoch(self, data_iter, models, num_personas, optimizers, schedulers, gradient_accumulation_steps, device, fp16, amp, \
        ...
        for i, batch in enumerate(data_iter):
            ...
            batch_response_topic_mask, batch_Uce_persona, batch_UPct_persona, batch_persona_topic_mask \
                = convert_topic_embedding_matrix(topic_embeddding, batch_context_topic_distribution, batch_response_topic_distribution, batch_persona_topic_distribution, device)
    ...
    def forward(self, batch_context_emb, batch_response_emb, batch_persona_emb, \
        ...
        # Interaction
        # 1. Only word level
        context_add_response_attn = self.context_transformer(batch_context_emb, batch_response_emb, batch_response_emb)
        response_add_context_attn = self.response_transformer(batch_response_emb, batch_context_emb, batch_context_emb)

        persona_add_response_attn = self.persona_transformer(batch_persona_emb, batch_response_emb, batch_response_emb)
        response_add_persona_attn = self.response_transformer(batch_response_emb, batch_persona_emb, batch_persona_emb)
        # 2. mask
        # 3. similarity matrix
        ...
        context_response_word_level_attn_simialrity_matrix = context_response_word_level_attn_simialrity_matrix.masked_fill_(context_response_attn_mask, 0)
        ...
        persona_response_word_level_attn_simialrity_matrix = persona_response_word_level_attn_simialrity_matrix.masked_fill_(persona_response_attn_mask, 0)

        output = self.cnn_block(context_response_attn_similarity_matrix, context_response_similarity_matrix,\
                                persona_response_attn_similarity_matrix, persona_response_similarity_matrix,\
                                context_response_word_level_attn_simialrity_matrix, persona_response_word_level_attn_simialrity_matrix)
...
def convert_topic_id_to_embedding(data, topic_embedding):
...
def convert_topic_embedding_matrix(topic_embedding, context_topic_distribution, response_topic_distribution, persona_topic_distribution, device):
    ...
    for context, response, persona in zip(context_topic_distribution, response_topic_distribution, persona_topic_distribution):
        context_topic_embedding_prob = convert_topic_id_to_embedding(context, topic_embedding)
        response_topic_embedding_prob = convert_topic_id_to_embedding(response, topic_embedding)
        persona_topic_embedding_prob = convert_topic_id_to_embedding(persona, topic_embedding)
