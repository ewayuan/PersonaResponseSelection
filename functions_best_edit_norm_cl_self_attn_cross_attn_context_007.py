...
class cnnBlock(nn.Module):
    def __init__(self):
        ...
        self.cnn_conv1_cr = nn.Conv2d(in_channels=4, out_channels=8, kernel_size=(3,3))
        self.maxpooling1_cr = nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2))

        self.cnn_conv2_cr = nn.Conv2d(in_channels=8, out_channels=16, kernel_size=(3,3))
        self.maxpooling2_cr = nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2))

        self.cnn_conv3_cr = nn.Conv2d(in_channels=16, out_channels=32, kernel_size=(3,3))
        self.maxpooling3_cr = nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2))
        
        self.relu_cr = nn.ReLU()
        self.dropout_cr = nn.Dropout(0.2)

        self.bn1_cr = nn.BatchNorm1d(num_features=768)
        self.fc1_cr = nn.Linear(in_features=1920, out_features = 768)

        # For Persona Response
        self.cnn_conv1_pr = nn.Conv2d(in_channels=4, out_channels=8, kernel_size=(3,3))
        self.maxpooling1_pr = nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2))

        self.cnn_conv2_pr = nn.Conv2d(in_channels=8, out_channels=16, kernel_size=(3,3))
        self.maxpooling2_pr = nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2))

        self.cnn_conv3_pr = nn.Conv2d(in_channels=16, out_channels=32, kernel_size=(3,3))
        self.maxpooling3_pr = nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2))

        self.relu_pr = nn.ReLU()
        self.dropout_pr = nn.Dropout(0.2)

        self.bn1_pr = nn.BatchNorm1d(num_features=768)
        self.fc1_pr = nn.Linear(in_features=1728, out_features = 768)
    ...
    def forward(self, context_response_emb_similarity_matrix, persona_response_emb_similarity_matrix, \
                context_response_self_similarity_matrix, persona_response_self_similarity_matrix,\
                context_response_attn_similarity_matrix, persona_response_attn_similarity_matrix,\
                context_response_word_level_attn_simialrity_matrix, persona_response_word_level_attn_simialrity_matrix):

        context_response_M = torch.stack([context_response_emb_similarity_matrix, context_response_self_similarity_matrix, \
                                          context_response_attn_similarity_matrix, context_response_word_level_attn_simialrity_matrix], dim=1)
        persona_response_M = torch.stack([persona_response_emb_similarity_matrix, persona_response_self_similarity_matrix,\
                                          persona_response_attn_similarity_matrix, persona_response_word_level_attn_simialrity_matrix], dim=1) 
...
class ourModel (nn.Module):
    def __init__(self, device):
        ...
        self.log_inv_t = torch.nn.Parameter(torch.tensor(1.0 / 0.07).log())
    ...
    def train_epoch(self, data_iter, models, num_personas, optimizers, schedulers, gradient_accumulation_steps, device, fp16, amp, \
        ...
        for i, batch in enumerate(data_iter):
            ...
            cl_labels = torch.arange(batch_size, device=batch[0].device)
            # cprint("targets: ", targets)
            # reporter.report()
            logits, cl_logits = context_model.module.forward (batch_context_emb, batch_response_emb, batch_persona_emb, \
                           ...
                           batch_size, num_candidates, device, "train")
            logits = logits.reshape(batch_size, num_candidates)
            cl_logits = cl_logits.reshape(batch_size, num_candidates)
            loss = F.binary_cross_entropy_with_logits(logits, targets) + F.cross_entropy(cl_logits, cl_labels)
            ...
            with torch.no_grad():
                ...
                for i in range(batch_size):
                    ...
                    logits_single = context_model (context_emb, batch_response_emb, persona_emb, \
                                          ...
                                          batch_size, num_candidates, device, "eval")
    ...
    def forward(self, batch_context_emb, batch_response_emb, batch_persona_emb, \
         ...
         batch_UPct_response, batch_Uce_persona, batch_UPct_persona, \
         batch_context_topic_mask, batch_response_topic_mask, batch_persona_topic_mask, \
         batch_size, num_candidates, device, type):
        ...
        context_response_emb_similarity_matrix = torch.bmm(batch_context_emb_norm, batch_response_emb_norm.transpose(1,2))
        persona_response_emb_similarity_matrix = torch.bmm(batch_persona_emb_norm, batch_response_emb_norm.transpose(1,2))
        ...
        context_self_attn_output = self.context_transformer(batch_context_emb, batch_context_emb, batch_context_emb, mask=context_self_mask.bool())
        context_self_attn_output_norm = torch.nn.functional.normalize(context_self_attn_output, dim=-1) 

        response_self_attn_output = self.response_transformer(batch_response_emb, batch_response_emb, batch_response_emb, mask=response_self_mask.bool())
        response_self_attn_output_norm = torch.nn.functional.normalize(response_self_attn_output, dim=-1) 
        
        persona_self_attn_output = self.persona_transformer(batch_persona_emb, batch_persona_emb, batch_persona_emb, mask=persona_self_mask.bool())
        persona_self_attn_output_norm = torch.nn.functional.normalize(persona_self_attn_output, dim=-1) 

        context_response_self_similarity_matrix = torch.bmm(context_self_attn_output_norm, response_self_attn_output_norm.transpose(1,2))
        persona_response_self_similarity_matrix = torch.bmm(persona_self_attn_output_norm, response_self_attn_output_norm.transpose(1,2))
        ...
        context_response_attn_similarity_matrix = torch.bmm(context_attn_output_norm, response_attn_output_norm.transpose(1,2))
        persona_response_attn_similarity_matrix = torch.bmm(persona_attn_output_norm, response_attn_output_norm.transpose(1,2))
        ...
        # 3. similarity matrix
        context_response_word_level_attn_simialrity_matrix = torch.bmm(context_add_response_attn_norm, response_add_context_attn_norm.transpose(1,2))
        persona_response_word_level_attn_simialrity_matrix = torch.bmm(persona_add_response_attn_norm, response_add_persona_attn_norm.transpose(1,2))

        logits = self.cnn_block(context_response_emb_similarity_matrix, persona_response_emb_similarity_matrix, \
                              context_response_self_similarity_matrix, persona_response_self_similarity_matrix,\
                              context_response_attn_similarity_matrix, persona_response_attn_similarity_matrix,\
        ...
        response_self_attn_output_unique = response_self_attn_output[:batch_size]
        size, max_len, emb = response_self_attn_output_unique.shape

        response_self_attn_output_ = response_self_attn_output_unique.sum(dim=1) / max_len
        response_self_attn_output_normal = F.normalize(response_self_attn_output_, dim=1)

        similarity_matrix_response_cl = []
        for i in range(size):
            response_add_context_attn_unique = response_add_context_attn[i * size: (i+1) * size]
            response_add_context_attn_ = response_add_context_attn_unique.sum(dim=1) / max_len
            response_add_context_attn_normal = F.normalize(response_add_context_attn_, dim=1)
            cur_similarity_matrix_response_cl = torch.matmul(response_self_attn_output_normal, response_add_context_attn_normal.T)
            similarity_matrix_response_cl.append(cur_similarity_matrix_response_cl[i])


        # similarity_matrix = torch.matmul(response_self_attn_output_normal, response_self_attn_output_normal.T)

        mask_cl = torch.eye(batch_size).to(device)
        cl_logits = torch.stack(similarity_matrix_response_cl)
        cl_logits *= self.log_inv_t.exp()

        return logits, cl_logits
